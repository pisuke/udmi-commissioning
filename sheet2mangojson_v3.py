...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="create a unique BACNET data source for each controller. Optionally specify a number to group controllers into a data source (e.g., -u 40).",
    )
    parser.add_argument(
        "-i",
        "--input",
    ...
    # Get a list of unique, valid device names
    unique_device_names = sorted([d['sanitized_device_name'] for _, d in devices_data.iterrows() if d.get('sanitized_device_name') not in ["BAC0"] and d.get('sanitized_device_name') in all_sheets_data])
    
    # Create a mapping from device name to data source XID
    device_to_ds_xid = {}
    
    if args.unique is not None:
        group_size = args.unique
        for i, device_name in enumerate(unique_device_names):
            # Group numbers should start from 1
            group_number = (i // group_size) + 1
            data_source_xid = f"DS_BACNET_Group_{group_number}"
            device_to_ds_xid[device_name] = data_source_xid
    else:
    ...
    # Now, loop through the devices_data to process each device and its points
    for _, device in devices_data.iterrows():
        ...
        # Get the correct data source XID from the map
        current_data_source_xid = device_to_ds_xid.get(sanitized_device_name, "DS_BACNET")
    ...
    # Use the unique data sources from the mapping for the JSON output
    unique_ds_xids = sorted(list(set(device_to_ds_xid.values())))
    data_source_sections = []
    for ds_xid in unique_ds_xids:
        # Change the naming to "BACnet device Group X"
        ds_name = ds_xid.replace("DS_BACNET_Group_", "BACnet device Group ")
        data_source_sections.append(TEMPLATE_BACNET_DATASOURCE.substitute(data_source_xid=ds_xid, data_source_name=ds_name))
