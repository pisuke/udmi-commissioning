...
OUT_FILE=$OUT_DIR/bacnet-scan-debian12
...
podman run --rm --network host --volume $ROOT_DIR/:/src --volume $TMP_DIR:/tmp pyinstaller-builder-debian-12:latest /bin/bash /tmp/build.sh
mkdir -p $OUT_DIR
